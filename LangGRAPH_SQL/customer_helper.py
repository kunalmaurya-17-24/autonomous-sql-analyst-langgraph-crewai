...
# Reasoning Model (Updated to 2.5)
model_pro = ChatGoogleGenerativeAI(temperature=0, model='gemini-2.5-flash')

# Speed Model (Flash)
model_flash = ChatGoogleGenerativeAI(temperature=0, model='gemini-2.5-flash')

# Aliasing for backward compatibility in the chain definitions
model = model_pro
model_llama = model_flash
...
CRITICAL: If the user asks about **spending, expenses, or budgets**, and a descriptive column (like 'merchant', 'description', 'store_name', 'seller_name') is available, **ALWAYS SELECT IT**. This is required to provide granular insights (e.g., "Top 3 merchants") even if the user didn't explicitly ask for a breakdown.
"""),
...
template_sql_query = ChatPromptTemplate.from_messages([
    ("system", """
You are an intelligent SQLite query generator. Your task is to generate syntactically correct and optimized SQLite queries based on the user's question, relevant table/column details, and optional filter values.
...
CRITICAL INSIGHT RULE: If the query involves analyzing **spending, expenses, or budgets**, and a descriptive column (like 'merchant_name', 'description', 'store_name') is present in the selected columns, do NOT just return a total sum.
Instead, you MUST **GROUP BY** that descriptive column and **ORDER BY** the expense amount (DESC) to show the Top 5 contributors.
Example: Instead of `SELECT SUM(amount)`, write `SELECT merchant_name, SUM(amount) ... GROUP BY merchant_name ORDER BY 2 DESC LIMIT 5`.
This enables the system to give specific advice (e.g., "Cut Starbucks") rather than generic advice.

NEGATIVE FILTER RULE (CRITICAL):
When calculating "spending", "expenses", "costs", or "over-budget", you MUST EXPLICITLY EXCLUDE positive cash flow categories.
ALWAYS add a filter: `WHERE category NOT IN ('Income', 'Salary', 'Deposit')` (or similar, check column values).
NEVER include the user's employer (e.g., 'Tech Corp Inc.') or income source as a 'Merchant' to cut.
"""),
...
template_validation = ChatPromptTemplate.from_messages([
    ("system", """
You are a highly capable and precise SQLite query validator.

Your role is to:
1. Understand the user's question.
2. Interpret the selected input columns and their descriptions.
3. **Cross-reference the generated SQL against `All Available Tables` (provided below)** to ensure NO hallucinated tables or columns are used.
4. Remove uncessary columns if any in the output. I need only relavant columns in output. Remove all unecessary columns from select.
5. Analyze the SQL query generated by a prior agent.
6. Validate the query strictly according to syntax, logic, and compliance rules.
7. Remove/REPLACE any reserved SQL keywords like 'or', 'and', or 'as' as aliases, as they may cause query errors in the given SQL
8. If the query involves filtering grouped results or counting grouped records, use subqueries where appropriate to avoid logical conflicts between GROUP BY, HAVING, and aggregate functions in the SELECT clause
...
**Relevant Tables and Columns (Selected by Agent 1):**  
{columns}

**All Available Tables (Full Schema for Hallucination Check):**
{all_table_info}
...
chain_query_validator = (
    RunnableMap({
        "columns": lambda x: x["columns"],
        "query": lambda x: x["user_query"],
        "filters": lambda x: x["filters"],
        'sql_query': lambda x: x["sql_query"],
        "all_table_info": lambda x: x["all_table_info"],
    })
    | template_validation
    | model
    | StrOutputParser()
